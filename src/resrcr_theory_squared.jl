...
function resrcr_theory_squared!(A, B, b::Vector{T}, x::Vector{T};  deter::Int64=0,
    ...
    for iter = deter + 1 : maxIter
        value_n = sqrt(res_list_A[iter]^2 - res_list_A[iter+1]^2) - sqrt(res_list_A[iter+1]^2 - res_list_A[iter+2]^2)
        p_p = value_n / value_d 
...
export CGData, CGData2, CGData3, resrcr_theory_squared!