...
function resrcr_theory!(A, B, b::Vector{T}, x::Vector{T};  deter::Int64=1, p0::Float64=0.5,
    ...
    value_d = sqrt(res_list_A[deter+1] - res_list_A[deter + 2])




    for iter = deter + 1 : maxIter
        value_n = sqrt(res_list_A[iter] - res_list_A[iter+1]) - sqrt(res_list_A[iter+1] - res_list_A[iter+2])
        p_p = value_n / value_d * (1-p0)
...
export CGData, CGData2, CGData3, resrcr_theory!