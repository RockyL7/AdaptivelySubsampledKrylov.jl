...
# A for the deterministic computation and B for the actual randomized computation
...
function resrcr_theory_c!(A, B, b::Vector{T}, x::Vector{T};  deter::Int64 = 1, p0::Float64=0.5,
    ...
    for iter = 0 : deter
        if (iter == deter)
            ...
            d = rand()
            if (d < p0)
                return 4, iter, res_list_A, x_B, P_list
            end
        end
        B(data.Bp, data.p_B)
        B(data.Br, data.r_B)
        ...
        genblas_axpy!(weight * alpha_B, data.p_B, x_B)
        # r -= alpha*Ap
    ...
    value_deno = 0
    while (iter <= maxIter)
        value_prev = res_list_A[iter] - res_list_A[iter + 1]
        value_curr = res_list_A[iter+1] - res_list_A[iter+2]

        while (value_prev < value_curr)
            A(data.Ap, data.p_A)
            A(data.Ar, data.r_A)
            gamma_A = genblas_dot(data.r_A, data.Ar)
            alpha_A = gamma_A / genblas_dot(data.Ap, data.Ap)
            ...
            # x += alpha*p
            hist_p .= hist_p + alpha_A * data.p_A
            genblas_axpy!(alpha_A, data.p_A, x)
            # r -= alpha*Ap
            genblas_axpy!(-alpha_A, data.Ap, data.r_A)
            rel_residual_A = genblas_nrm2(data.r_A) / norm_b
            ...
            term = term + 1
            iter = iter + 1
            value_prev = value_prev * (term-1) / term + value_curr / term
            value_curr = res_list_A[iter+1] - res_list_A[iter+2]
        end
            ...
            if (pointer == 1)
                value_deno = value_prev
            end

            p_p = (sqrt(value_prev) - sqrt(value_curr)) / sqrt(value_deno) * (1-p0)
            P_list = hcat(P_list, p_p)
            Random.seed!(7 * (iter - term + 1 +1) + 1 + 35 * seed)
            d = rand()

            if (d < (p_p / (1 - sum_p)))
                return -13, iter - term + 1, res_list_A, x_B, P_list
            end
...
export CGData, CGData2, CGData3, resrcr_theory_c!